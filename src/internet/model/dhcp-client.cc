...
//                   MakeTimeAccessor (&DhcpClient::m_interval),
//                   MakeTimeChecker ())
    .AddAttribute (
      "RemoteAddress",
...
void DhcpClient::LinkStateHandler(void){
   ...
   if(GetNode()->GetDevice(0)->IsLinkUp()){  //assumes a single net device on node !!!
    ...
    m_socket->SetRecvCallback(MakeCallback(&DhcpClient::NetHandler, this)); 
//      m_refreshEvent = Simulator::Schedule (Seconds (1.0), &DhcpClient::RunEfsm, this);
    RunEfsm();
   }
   else{
...
//        NS_LOG_INFO("AICICIIII!=" <<  Mac48Address::ConvertFrom(GetNode()->GetDevice(0)->GetAddress()));
        ...
        m_state = WAIT_OFFER;
        m_socket->SetRecvCallback(MakeCallback(&DhcpClient::NetHandler, this)); 
        m_rtrsEvent = Simulator::Schedule (Seconds (RTRS_TIMEOUT), &DhcpClient::RtrsHandler, this);
        break;
...
//                NS_LOG_INFO (Mac48Address::ConvertFrom(GetNode()->GetDevice(0)->GetAddress()));
                 break; //this offer is not for me 
                }
...
packet = Create<Packet> (DHCP_HEADER_LENGTH);
        ...
        header.SetChaddr(Mac48Address::ConvertFrom(GetNode()->GetDevice(0)->GetAddress()));
        packet->AddHeader (header);
                  NS_LOG_INFO ("[node " << GetNode()->GetId() << "]  "<< "Trace TX: DHCP ADDRESS"<<Mac48Address::ConvertFrom(GetNode()->GetDevice(0)->GetAddress()));
//        NS_LOG_INFO("AICICIIII!=" <<  Mac48Address::ConvertFrom(GetNode()->GetDevice(0)->GetAddress()));
...
//                NS_LOG_INFO (Mac48Address::ConvertFrom(GetNode()->GetDevice(0)->GetAddress()));
                            NS_LOG_INFO ("[node " << GetNode()->GetId() << "]  "<< "Trace TX: DHCP ADDRESS"<<Mac48Address::ConvertFrom(GetNode()->GetDevice(0)->GetAddress()));
NS_LOG_INFO ("[node " << GetNode()->GetId() << "]  "<< "Trace TX: DHCP ADDRESS RECV"<<header.GetChaddr());
                ...
                ipv4->RemoveAddress(ifIndex, 0); //it is assumed this node has a single IP address settled ! 
                
                ipv4->AddAddress (ifIndex, Ipv4InterfaceAddress ((Ipv4Address)m_myAddress, Ipv4Mask ("/24"))); //BEAWARE: the node IP mask is hardcoded here !!!!!!!!!!!!!
                ipv4->SetUp (ifIndex); 
                ...
                Ipv4StaticRoutingHelper ipv4RoutingHelper; //setup the gw IP addr
                ...
                ipv4->SetUp (0); //the loopback dev i/f status is changing whenever link is changing !!!!
...
//                NS_LOG_INFO (Mac48Address::ConvertFrom(GetNode()->GetDevice(0)->GetAddress()));
    ...
    case REFRESH_LEASE:
        ...
        //here to add code to deal with DHCP ACK!!!
